...
import { Archive, ArrowUpRight, BookOpen, Check, Clock3, Plus, Send, Sparkles, X } from "lucide-react";
...
export function PlansPanel({ data, onReload, onEdit, onCreate }: { data: HrWorkspace; onReload: () => Promise<void>; onEdit: (plan: PlanView) => void; onCreate: () => void }) {
  ...
  async function act(id: string, action: string, response?: string) {
    setError(""); setBusy(id);
    try { await hrRequest("/plans", { id, action, response }); await onReload(); }
  ...
  return <section className="space-y-5">
    ...
    <div className="grid grid-cols-2 gap-5">{plans.map((plan) => <article key={plan.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6">
      ...
      <ol className="my-5 space-y-3">{plan.steps.map((step, i) => { const done = plan.completedEventIds.includes(step.eventId); return <li key={step.eventId} className="flex gap-3"><span className={`flex size-6 shrink-0 items-center justify-center rounded-full text-xs ${done ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-500"}`}>{done ? <Check className="size-3" /> : i + 1}</span><div><p className="text-sm font-medium">{data.events.find((e) => e.event_id === step.eventId)?.title ?? step.eventId}</p><p className="mt-1 text-xs text-slate-500">{done ? "Завершено" : `До ${dateLabel(step.dueDate)}`}</p></div></li>; })}</ol>
      ...
      <div className="mt-auto border-t border-slate-100 pt-4"><div className="flex justify-between text-xs text-slate-500"><span>Пройдено {plan.completedEventIds.length} из {plan.steps.length}</span><span>{plan.response === "accepted" ? "Сотрудник принял план" : plan.response === "declined" ? "Сотрудник выбрал «Не сейчас»" : plan.state === "published" ? "Ожидает ответа сотрудника" : "Доступен только авторизованной команде"}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-emerald-600" style={{ width: `${plan.steps.length ? plan.completedEventIds.length / plan.steps.length * 100 : 0}%` }} /></div>
        <div className="mt-4 flex flex-wrap gap-2">{data.account.permissions.plans && plan.state !== "archived" && <>{plan.state === "draft" && <><Button variant="outline" disabled={busy === plan.id} onClick={() => onEdit(plan)}>Изменить</Button><Button disabled={busy === plan.id} onClick={() => void act(plan.id, "publish")}><Send />Опубликовать</Button></>}<Button variant="ghost" disabled={busy === plan.id} onClick={() => void act(plan.id, "archive")}><Archive />В архив</Button></>}{data.account.role === "employee" && <><Button disabled={busy === plan.id || plan.response === "accepted"} onClick={() => void act(plan.id, "respond", "accepted")}><Check />Принять план</Button><Button variant="outline" disabled={busy === plan.id || plan.response === "declined"} onClick={() => void act(plan.id, "respond", "declined")}>Не сейчас</Button><Link href="/" className="ml-auto flex items-center gap-1 text-sm text-emerald-700">В город<ArrowUpRight className="size-4" /></Link></>}</div>
      </div>
    </article>)}</div>
    <p className="text-xs text-slate-500">Выполнение обновляется из истории учебных активностей. Публикация плана не начисляет навыки.</p>
  </section>;
}
...
export function PlanEditor({ employee, plan, data, onSaved, onClose }: { employee: EmployeeView; plan?: PlanView; data: HrWorkspace; onSaved: () => Promise<void>; onClose: () => void }) {
  ...
  const hours = steps.reduce((sum, s) => sum + (candidates.find((c) => c.eventId === s.eventId)?.hours ?? 0), 0);
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30" role="dialog" aria-modal="true" aria-labelledby="plan-heading" onKeyDown={(e) => { if (e.key === "Escape") onClose(); }}>
    <form className="flex h-full w-[760px] flex-col bg-white shadow-2xl" onSubmit={async (e) => {
      e.preventDefault(); setBusy(true); setError("");
      try { await hrRequest("/plans", { action: plan ? "update" : "create", id: plan?.id, employeeId: employee.id, title, goal, note, steps }); await onSaved(); }
      catch (err) { setError(err instanceof Error ? err.message : "Не удалось сохранить"); } finally { setBusy(false); }
    }}>
      <header className="flex items-start justify-between border-b p-6"><div><p className="text-xs font-medium uppercase tracking-widest text-emerald-700">Индивидуальное развитие</p><h2 id="plan-heading" className="mt-2 text-xl font-semibold">{plan ? "Изменить план обучения" : "Новый план обучения"}</h2><p className="mt-1 text-sm text-slate-500">{employee.name} · {employee.role} · {employee.grade}</p></div><Button type="button" variant="ghost" size="icon" aria-label="Закрыть план" onClick={onClose}><X /></Button></header>
      <div className="flex-1 space-y-5 overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between"><h3 className="font-semibold">Подходящие активности</h3><Button type="button" variant="outline" disabled={loading || !available.length} onClick={() => setSteps(available.slice(0, 3).map((c) => ({ eventId: c.eventId, dueDate: deadline(c) })))}><Sparkles />Подобрать до 3 шагов</Button></div>
        ...
        <div className="space-y-3">{available.map((c) => { const selected = steps.find((s) => s.eventId === c.eventId); return <div key={c.eventId} className={`rounded-xl border p-4 ${selected ? "border-emerald-400 bg-emerald-50/40" : "border-slate-200"}`}><label className="flex items-start gap-3"><input type="checkbox" checked={!!selected} className="mt-1 accent-emerald-700" onChange={(e) => setSteps(e.target.checked ? [...steps, { eventId: c.eventId, dueDate: deadline(c) }] : steps.filter((s) => s.eventId !== c.eventId))} /><div className="flex-1"><p className="text-sm font-semibold">{c.title}</p><p className="mt-1 text-xs text-slate-500">{c.hours} ч · {c.format === "online" ? "Онлайн" : c.format === "offline" ? "Очно" : "В своём темпе"}{c.nextSession ? ` · с ${dateLabel(c.nextSession)}` : ""}</p></div></label><ul className="ml-7 mt-3 space-y-1 text-xs leading-relaxed text-slate-600">{c.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul>{selected && <label className="ml-7 mt-3 flex items-center gap-3 text-xs text-slate-600">Завершить до<input aria-label={`Срок: ${c.title}`} type="date" required min={c.nextSession?.slice(0, 10) ?? data.asOf} value={selected.dueDate} onChange={(e) => setSteps(steps.map((s) => s.eventId === c.eventId ? { ...s, dueDate: e.target.value } : s))} className={`${fieldClass} !w-44`} /></label>}</div>; })}</div>
      ...
      <footer className="space-y-3 border-t p-6">{error && <p role="alert" className="text-sm text-red-700">{error}</p>}<div className="flex items-center justify-between"><span className="text-sm text-slate-500">{steps.length} активностей · {hours} ч</span><div className="flex gap-2"><Button type="button" variant="ghost" onClick={onClose}>Отмена</Button><Button type="submit" disabled={busy || loading || !steps.length}>Сохранить черновик</Button></div></div><p className="text-xs text-slate-400">После публикации сотрудник сможет принять план или выбрать «Не сейчас».</p></footer>
    </form>
  </div>;
}
