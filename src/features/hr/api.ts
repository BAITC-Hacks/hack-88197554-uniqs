...
function planView(plan: LearningPlan): PlanView {
  ...
  return { ...plan, employeeName: employee.full_name, department: employee.department, completedEventIds: plan.steps.filter((s) => completed.has(s.eventId)).map((s) => s.eventId), totalHours: plan.steps.reduce((sum, s) => sum + (events.find((e) => e.event_id === s.eventId)?.duration_hours ?? 0), 0) };
}
