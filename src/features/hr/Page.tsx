...
function Workspace({ account, onLogout }: { account: Account; onLogout: () => void }) {
  ...
  return <main className="min-h-screen bg-[#f4f6f5] text-slate-900">
    <aside className="fixed inset-y-0 left-0 flex w-60 flex-col border-r border-slate-200 bg-white px-5 py-7">
      ...
      <nav aria-label="Разделы HR" className="space-y-2">{[{ id: "overview", name: "Обзор компетенций", icon: ChartNoAxesCombined }, { id: "plans", name: "Планы обучения", icon: GraduationCap }, ...(viewer.permissions.plans ? [{ id: "templates", name: "Шаблоны обучения", icon: Layers3 }] : []), ...(viewer.permissions.access ? [{ id: "access", name: "Доступ и роли", icon: ShieldCheck }] : [])].map(({ id, name, icon: Icon }) => <button key={id} type="button" onClick={() => setTab(id)} aria-current={activeTab === id ? "page" : undefined} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm transition-colors ${activeTab === id ? "bg-emerald-50 font-medium text-emerald-800" : "text-slate-500 hover:bg-slate-50"}`}><Icon className="size-4" />{name}</button>)}</nav>
