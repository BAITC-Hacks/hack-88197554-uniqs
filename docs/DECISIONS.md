...
- 2026-09-23 — city (open-world) — Модели: KayKit (City Builder Bits, Furniture Bits, Restaurant Bits, Character Pack Adventurers), все CC0, лицензии в `public/models/*/License.txt`; персонаж по грейду = класс приключенца (Rogue → Knight → Mage → Barbarian), оружие скрыто. — kenney.nl из сети недоступен, KayKit лежит на GitHub, один стиль и есть анимации idle / walk / run / sit.
